...
<style>
html {
	background: url('images/reviewbgi.jpg') center center/cover no-repeat
		fixed;
}

body {
	background: linear-gradient(rgba(250, 246, 238, .88),
		rgba(250, 246, 238, .88));
	color: #2b2b2b;
...
<body>
	...
	<div class="container py-5">
		<div
			...
			<div>
				...
				<p>Share your vivid review of the mountain you visited.</p>
			</div>
			<button type="button" class="btn btn-info" id="btnWrite">
				<i class="bi bi-pencil-square me-1"></i> 리뷰 작성하기 / write review
			</button>
		</div>
		...
		<div id="reviewBody">
			<c:choose>
				<c:when test="${empty reviewlist}">
					<div class="empty-state">
						<i class="bi bi-chat-square-text"></i> 🥾 아직 작성된 리뷰가 없습니다. 첫 리뷰를
						남겨보세요! <br /> No reviews yet. Be the first to leave one!
							...
							<div class="col-md-6">
								<div class="review-card" data-id="${rv.rvNumid}">
									<div class="d-flex justify-content-between align-items-start">
										<p class="review-title mb-2">
											&ldquo;
											<c:out value="${rv.rvTitle}" />
											&rdquo;
										</p>
									...
									<div class="review-meta">
										<span class="review-avatar"><c:out
												value="${fn:substring(rv.rvNickid, 0, 1)}" /></span> <span
											class="review-nick"><c:out value="${rv.rvNickid}" /></span>
										<c:if test="${not empty rv.mtName}">
											<span class="badge mt-badge ms-auto"><i
												class="bi bi-geo-alt-fill me-1"></i>
											<c:out value="${rv.mtName} ${rv.mtNameEn}" /></span>
										</c:if>
									</div>
								</div>
							</div>
			...
			<div class="text-center mt-4">
						<a href="main.do"
							...
							class="bi bi-arrow-left me-1"></i> 돌아가기 / Back
						</a>
					</div>
		</div>
	</div>
	...
	<script>
      $(function() {
         ...
         let reviewModal = new bootstrap.Modal(reviewModalEl);
         ...
         function resetForm() {
            ...
            $('#reviewModalLabel').text('리뷰 작성/ write review');
         }
         ...
         $(document).on('click', '.btn-edit', function() {
            ...
            $.ajax({
               ...
               success : function(data) {
                  ...
                  if (data.rvImg) {
                     $('#imgPreview').attr('src', data.rvImg).removeClass('d-none');
                  }
                  $('#mtId').val(data.mtId);
                  $('#rvPwd').val('');
                  $('#reviewModalLabel').text('리뷰 수정 / edit review');
                  reviewModal.show();
               },
               error : function() {
         ...
         $(document).on('click', '.btn-delete', function() {
            let rvNumid = $(this).data('id');
            let pwd = prompt('삭제하려면 비밀번호를 입력하세요 / Enter your password to delete:');
         ...
         $('#btnSave').on('click', function() {
            ...
            let rvTitle = $('#rvTitle').val().trim();
            let rvContent = $('#rvContent').val().trim();
            let rvNickid = $('#rvNickid').val().trim();
            let rvPwd = $('#rvPwd').val();
            let mtId = $('#mtId').val();
            ...
            let rvNumid = $('#rvNumid').val();
            let isEdit = rvNumid !== '';
            let url = isEdit ? 'edit.do' : 'insert.do';
